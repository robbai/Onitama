...
void init_move_tables() {
    // Create a list of "DELTAS", that represent each of the cards' moves from origin.
    ...
    for (int square = 0; square < SQUARE_NUM; ++square) {
        ...
        for (int card = 0; card < CARD_NUM; ++card) {
            for (vector<int> delta : DELTAS[card]) {
                // White's perspective.
                int card_file = file + delta[0],
                        card_rank = rank + delta[1];
                if (on_board(card_file, card_rank)) {
                    MOVE_TABLES[card][square][0] |= 1 << (card_file + card_rank * BOARD_LENGTH);
                }

                // Black's perspective.
                card_file = file - delta[0],
                        card_rank = rank - delta[1];
                if (on_board(card_file, card_rank)) {
                    MOVE_TABLES[card][square][1] |= 1 << (card_file + card_rank * BOARD_LENGTH);
                }
            }
        }
    }
